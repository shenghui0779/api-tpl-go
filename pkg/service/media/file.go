...
func File(w http.ResponseWriter, r *http.Request) {
	...
	record, err := db.Client().Media.Query().WithFile(func(fq *ent.FileQuery) {
		fq.Unique(false)
	...
	if err != nil {
		if ent.IsNotFound(err) {
			...
			log.Error(ctx, "Error query media", zap.Error(err), zap.String("media_id", mediaID))
	...
	if query.Has("info") {
		result.OK(result.V(&MediaInfo{
			...
			Width:       file.Width,
			Height:      file.Height,
			Orientation: consts.Orientation(file.Orientation).String(),
	...
	filePath := internal.MediaFile(file.Fingerprint, record.FileName)
